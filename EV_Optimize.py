...
def input_params():
    ...
    if (width_current < 0) or (height_current < 0) or (poiQuantity_current < 0) or (csQuantity_current < 0) or (csNewQuantity_current < 0):
    ...
    if (poiQuantity_current > width_current*height_current) or (csQuantity_current + csNewQuantity_current > width_current*height_current):
...
def calc_length(param_1, param_2):
    return (param_1[0]**2 - 2*param_1[0]*param_2[0] + param_2[0]**2) + (param_1[1]**2 - 2*param_1[1]*param_2[1] + param_2[1]**2)
    ...
    if poiQuantity > 0:
        ...
        for poi_cur in pois_place:
            for i, cs_new in enumerate(csNew_place):
                poi_average_distance[i] += sum(abs(param_1 - param_2) for param_1, param_2 in zip(cs_new, poi_cur)) / poiQuantity
        print("Среднее расстояние до POI:\t\t\t", poi_average_distance)

    if csQuantity > 0:        
        old_cs_average_distance = [sum(abs(param_1 - param_2) for param_1, param_2 in zip(new_cs, cs_cur) for cs_cur in cs_place) / csQuantity for new_cs in csNew_place]
    ...
    if csNewQuantity > 1:
        ...
        for i in range(csNewQuantity):
            for j in range(i+1, csNewQuantity):
                new_cs_distance += abs(csNew_place[i][0]-csNew_place[j][0])+abs(csNew_place[i][1]-csNew_place[j][1])
...
if __name__ == '__main__':
    ...
    Net_Graph, pois_place, cs_place, csNewPotential_place = model_settings(params.width, params.height, params.poi, params.chargers)
